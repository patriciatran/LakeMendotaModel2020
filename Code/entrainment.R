...
layer.to.plot1 <- c("epi","hypo","mixed") # whole water column

layer.to.plot2 <- c("epi","mixed") # epi layer only

layer.to.plot.list <- list(layer.to.plot1, layer.to.plot2)

# variables I've plotted: start with those that are mg/L:
vars.to.plot.list <- c("nh4_sloh", "no3no2_sloh", "totpuf_sloh", "drp_sloh")


for (i in 1:length(layer.to.plot.list)){
  layer.to.plot <- layer.to.plot.list[i][[1]]
  for (j in 1:length(vars.to.plot.list)){
    ...
    var.to.plot <- vars.to.plot.list[j]
